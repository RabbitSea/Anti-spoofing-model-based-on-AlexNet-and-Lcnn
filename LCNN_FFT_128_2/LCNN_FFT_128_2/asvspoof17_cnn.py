...
from LCNN_FFT_128_2.asv2017_Dataset import ASVspoofTestData, ASVspoofTrainData, ASVspoofDevData
from LCNN_FFT_128_2.a_LCNN_net import Net_cqt
...
# scheduler = lr_scheduler.StepLR(step_size=25, optimizer=optimizer, gamma=0.1)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):  # 迭代到train_loader里面取数据,随机取
        ...
        if batch_idx % args.log_interval == 0:
            # if batch_idx % 1 == 0:
            process_rate = batch_idx * args.batch_size / 3014
            print('batch_idx = {}'.format(batch_idx))
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * args.batch_size, len(train_loader.dataset),
                process_rate, loss.item()))

    print("Train average loss = %.6f" % (total_loss / len(train_loader.dataset)))

    dev_correct = 0
    dev_loss = 0.
    model.eval()
    f = open('lcnn_cqt_dev-eer', 'w')
    ...
    print("Dev average loss is %.6f" % dev_loss)
...
if args.train:
    for epoch in range(1, args.epochs+1):
        ...
        torch.save(model, model_output_dir + 'lcnn_cqt_iter' + str(epoch) + '.mdl')
        shutil.copy('lcnn_cqt_dev-eer', model_output_dir + '/lcnn_cqt_dev-eer-' + str(epoch))

    # torch.save(model.state_dict(), model_output_dir + 'lcnn_final.mdl')
    torch.save(model, model_output_dir + 'lcnn_cqt_final.mdl')
...
print('EER of dev:')
cm_eer_dev = asv17_util.compute_eer17_from_file('lcnn_cqt_dev-eer')
print('EER of eval:')
cm_eer_eval = asv17_util.compute_eer17_from_file('lcnn-cqt-eer-file')
