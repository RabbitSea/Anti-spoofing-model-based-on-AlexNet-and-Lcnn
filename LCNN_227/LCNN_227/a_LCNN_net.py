...
class Net2(nn.Module):  # 标准lcnn
    def __init__(self):
        super(Net2, self).__init__()
        self.mfm = torch.nn.MaxPool3d(kernel_size=(2, 1, 1), stride=(2, 1, 1))
        # 1
        self.conv1 = nn.Conv2d(1, 32, kernel_size=5, stride=(1, 1))  # input，output, kernel   # 32
    ...
    def forward(self, x):
        ...
        x = self.mfm(self.conv2a(x))
        x = self.bn2(self.pool2(self.mfm(self.conv2(x))))
        x = F.dropout(x, p=0.5, training=self.training)

        x = self.mfm(self.conv3a(x))
        x = self.bn3(self.pool3(self.mfm(self.conv3(x))))
        x = F.dropout(x, p=0.5, training=self.training)
