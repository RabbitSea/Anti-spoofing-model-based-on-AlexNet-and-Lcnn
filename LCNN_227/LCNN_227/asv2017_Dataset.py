...
class ASVspoofTestData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print('Eval: index: {}， wav_name: {}'.format(idx, wav_name))
        # wav_name = wav_name.replace('.wav', '')
        feature = f[wav_name][:]
        feature = precess_feat_cnn(feature, nFeatures=feat_dim, nTime=time_dim)
...
def precess_feat_cnn(feat_in, nFeatures, nTime):  # 梅尔倒谱系数
    feat = feat_in
    ...
    feat = scale(feat, axis=1)  # 数据标准化,按帧
