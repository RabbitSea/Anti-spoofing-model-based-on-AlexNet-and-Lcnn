...
import shutil
import time
'''迁移alexnet，trained on train set and test set'''
...
model.classifier = torch.nn.Sequential(
    ...
    torch.nn.ReLU(),
    torch.nn.Dropout(p=0.7),  # 当dropout=0.3时过拟合较严重
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):  # 迭代到train_loader里面取数据,随机取
        ...
        if batch_idx % args.log_interval == 0:
            process_rate = batch_idx * args.batch_size / len(train_loader)  # 第一个打印的是第0个batch, 前一百个数据的loss
            print('batch_idx = {}'.format(batch_idx))
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(  # 最后只剩下14个样本，所以loss很小
                epoch, batch_idx * args.batch_size, len(train_loader.dataset),
                       process_rate, loss.item()))

    print("Train total loss = %.6f" % (total_loss / len(train_loader.dataset)))
...
def test():
    ...
    for data, target in test_loader:
        ...
        pred = output.data.max(1, keepdim=True)[1]  # get the index of the max log-probability
        correct += pred.eq(target.data.view_as(pred)).cpu().sum()  # view_as 确保维度一致
...
if args.train:
    ...
    torch.save(model, model_output_dir + 'alexnet_2.mdl')
...
print('EER of eval:')
cm_eer_eval = asv17_util.compute_eer17_from_file('eer-file-train-dev')


# model = torch.load('iter19.mdl')
# test()
# print('EER of eval:')
# cm_eer_eval = asv17_util.compute_eer17_from_file('eer-file-train-dev-6')
